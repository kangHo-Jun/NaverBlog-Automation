...
function onOpen() {
  try {
    SpreadsheetApp.getUi()
      ...
      .addItem('📁 직접업로드 모드', 'setModeManual_')
      .addSeparator()
...
function storeAutoGeneratedImageUrls_(sheet, rowIndex, geminiUrl, openaiUrl, svgUrl) {
  if (!sheet || !rowIndex) return;
  if (geminiUrl) {
...
function setModeAuto_() {
  setImageSourceMode_('자동생성', '자동생성 모드: Gemini + OpenAI 이미지 자동 생성');
}
...
function processAutoImage_(finalData, imageFolder) {
  ...
  var image1Choice = String((controlRow && controlRow.values ? controlRow.values[13] : '') || '').trim().toUpperCase();
  var storedGeminiUrl = String((controlRow && controlRow.values ? controlRow.values[11] : '') || '').trim();
      ...
      if (sheet) {
        ...
        if (imageSource === '자동생성') {
          if (prompt1) {
            ...
            Logger.log('⚠️ runGenerateOnly: image1 prompt가 없어 L/M/N열 URL 저장을 건너뜁니다.');
          }
...
function runPublishOnly() {
  try {
    ...
    if (imageSource === '자동생성') {
      Logger.log('🎨 자동생성 모드: Gemini + OpenAI + SVG 처리');
      imageResult = processAutoImage_(finalData, imageFolder);
    } else {
      Logger.log('🖼️ 직접업로드 모드: 임시 이미지 폴더를 생성합니다.');
      var uploadedCount = countNumberedImageFiles_(imageFolder);
...
function resetForNewPost() {
  ...
  sheet.getRange('L2').clearContent(); // image1_gemini_url
  sheet.getRange('M2').clearContent(); // image1_openai_url
  sheet.getRange('N2').clearContent(); // image1_선택
  ...
  SpreadsheetApp.getActiveSpreadsheet().toast(
    '초기화 완료! 모드를 선택하고 새 파일을 업로드하세요.', '대산 블로그', 5);
