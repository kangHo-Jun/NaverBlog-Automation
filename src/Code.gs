      ...
      if (sheet) {
        ...
        if (imageSource === '자동생성') {
          if (prompt1) {
            ...
            Logger.log('🖼️ image1 OpenAI URL: ' + (openAiImage1 && openAiImage1.publicUrl ? openAiImage1.publicUrl : '없음'));
            Logger.log('🖼️ image2 SVG 후보 URL: ' + (image2SvgUrl || '없음'));
          } else {
            Logger.log('⚠️ runGenerateOnly: image1 prompt가 없어 M/R열 URL 저장을 건너뜁니다.');
          }
