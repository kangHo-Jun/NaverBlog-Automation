...
function runPublishOnly() {
  try {
    ...
    if (imageSource === '자동생성') {
      ...
      downloadedPhotos = downloadUnsplashPhotosToFolder_(finalData.content, finalData, title);
      Logger.log('🧾 Unsplash 검색 결과 수: ' + downloadedPhotos.length);
      if (placeholders.length > 0 && downloadedPhotos.length === 0) {
        throw new Error('Unsplash 이미지 검색 0건: 플레이스홀더는 존재하지만 URL 확보에 실패했습니다. 로그의 검색어/응답 코드를 확인하세요.');
      }
    } else {
    ...
    mappedContent = imageSource === '자동생성'
      ? mapPhotosToPlaceholders(finalData.content, '', finalData, title)
...
function mapPhotosToPlaceholders(docContent, imageFolderId, finalData, title) {
  ...
  try {
    if (!imageFolderId) {
      Logger.log("🧠 자동생성 모드: Unsplash 원본 URL을 본문에 직접 삽입합니다.");
...
function searchUnsplashPhoto_(keyword) {
  ...
  var photo = responseData && responseData.results && responseData.results.length > 0
    ...
    : null;
  var imageUrl = photo && photo.urls ? (photo.urls.raw || photo.urls.full || photo.urls.regular || photo.urls.small || '') : '';
...
function buildUnsplashImageTag_(url, creditText) {
  if (!url) return '';

  var altText = String(creditText || 'Unsplash image')
    .replace(/&/g, '&amp;')
    .replace(/"/g, '&quot;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;');

  return '<img src="' + url + '" style="width:100%; max-width:800px; height:auto; display:block; margin:1em auto;" alt="' + altText + '">';
}
...
function downloadUnsplashPhotosToFolder_(docContent, finalData, title) {
  var placeholders = extractPhotoPlaceholders_(docContent);
  var downloaded = [];
  var queryCache = {};

  Logger.log('🧠 Unsplash CDN URL 수집 시작');
  Logger.log('📋 대상 플레이스홀더 수: ' + placeholders.length);

  for (var i = 0; i < placeholders.length; i++) {
    var holder = placeholders[i];
    var query = buildUnsplashKeywordFromHolder_(holder, finalData, title);
    Logger.log('🔎 URL 조회 시도: ' + holder + ' / query=' + query);
    ...
    downloaded.push({
      ...
      credit: searchResult.creditText
    });

    Logger.log('🔗 Unsplash URL 확보 완료: ' + holder + ' / ' + searchResult.creditText);
  }

  Logger.log('✅ Unsplash CDN URL 수집 완료: ' + downloaded.length + '개');
  return downloaded;
}
  ...
  try {
    ...
    for (var i = 0; i < uniqueHolders.length; i++) {
      ...
      if (searchResult.imageUrl) {
        var escapedHolder = holder.replace(/[\[\]]/g, "\\$&");
        var regex = new RegExp(escapedHolder, "g");
        resultBody = resultBody.replace(regex, buildUnsplashImageTag_(searchResult.imageUrl, searchResult.creditText));
        mappedCount++;
...
function convertToBloggerHTML(docContent, title) {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    // 이미지 URL 처리 (Drive URL 패턴 감지) - length 제한 제거
    else if (line.indexOf('https://drive.google.com/uc') !== -1 || line.indexOf('https://images.unsplash.com/') !== -1) {
